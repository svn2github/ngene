...
void MyInputListener::handleEvent(const KeyboardEvent& _evt)
{
	...
	{
		Physics::getSingleton().getWorld(L"MyPhysicsWorld")->toggleEnabled();
	}

	if(_evt.isKeyPressed(KC_N))
		m_pApp->selectNextActor();

	if(_evt.isKeyDown(KC_F))
	{
		FORCE_DESC force = {Vector3::UNIT_Y, 10000.0f};
		m_pApp->getSelectedActor()->applyForce(force);
	}
